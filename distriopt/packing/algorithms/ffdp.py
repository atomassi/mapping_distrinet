...
class FirstFitDecreasingPriority(PackingSolver):
    ...
    def solve(self, **kwargs):
        ...
        sorted_items = sorted(self.virtual.nodes(),
                              key=lambda x: self.virtual.req_cores(x) * 1000 + self.virtual.req_memory(x), reverse=True)

        # coefficient
        alpha = {'cores': sum(self.virtual.req_cores(u) for u in self.virtual.nodes()) / float(
            sum(self.physical.cores(vm) for vm in self.physical.vm_options)),
                 'memory': sum(self.virtual.req_memory(u) for u in self.virtual.nodes()) / float(
        ...
        for u in sorted_items:
            ...
            try:
                ...
                bin_to_open = next((bin_type for bin_type in sorted_bin_types if
                                    req_cores <= self.physical.cores(bin_type) and req_memory <= self.physical.memory(
                                        bin_type)), None)
