...
class FirstFitOrderedDeviation(PackingSolver):
    ...
    def solve(self, **kwargs):
        ...
        sorted_items = sorted(self.virtual.nodes(),
                              key=lambda u: abs(self.virtual.req_memory(u) - 1000 * self.virtual.req_cores(u)) / (
                    ...
                    ((bin, self.physical.hourly_cost(bin.vm_type) *
                      (max(
                          ...
                          req_memory / float(self.physical.memory(bin.vm_type)) - bin.used_cores / float(
                              self.physical.cores(bin.vm_type)) + bin.used_memory / float(
                              self.physical.memory(bin.vm_type))
                      )
                       if bin.used_cores / float(self.physical.cores(bin.vm_type)) > bin.used_memory / float(
                          self.physical.memory(bin.vm_type))
                       else
                       max(req_memory / float(self.physical.memory(bin.vm_type)),
                           req_cores / float(self.physical.cores(bin.vm_type)) + bin.used_cores / float(
                               self.physical.cores(bin.vm_type)) - bin.used_memory / float(
                               self.physical.memory(bin.vm_type)))))
                     for bin in bins if req_cores + bin.used_cores <= self.physical.cores(
                        bin.vm_type) and req_memory + bin.used_memory <= self.physical.memory(bin.vm_type)),
                    key=lambda x: x[1])

            except ValueError:
                cheapest_opened = None

            if cheapest_opened is not None and cost_cheapest_opened <= cost_cheapest_new:
