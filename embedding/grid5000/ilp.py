...
class EmbedILP(Embed):
    ...
    def __call__(self, virtual, physical, **kwargs):
        ...
        link_mapping = pulp.LpVariable.dicts("link_mapping",
                                             ...
                                             cat=pulp.LpContinuous if physical.grouped_interfaces else pulp.LpBinary)

        # node mapping variables
        node_mapping = pulp.LpVariable.dicts("node_mapping",
                                             ((u, i) for u in virtual.nodes() for i in physical.nodes()),
                                             cat=pulp.LpBinary)
        ...
        for u in virtual.nodes():
            mapping_ILP += pulp.lpSum(
                node_mapping[(u, i)] for i in physical.nodes()) == 1, f"assignment of {u} to a physical node"

        # Node capacity constraints
        for i in physical.nodes():
            # CPU limit
            mapping_ILP += pulp.lpSum(
                virtual.req_cores(u) * node_mapping[(u, i)] for u in virtual.nodes()) <= \
                           physical.cores(i), f"CPU capacity for physical node {i}"
            # Memory limit
            mapping_ILP += pulp.lpSum(
                virtual.req_memory(u) * node_mapping[(u, i)] for u in virtual.nodes()) <= \
                           physical.memory(i), f"memory capacity for physical node {i}"
        ...
        for (u, v) in virtual.sorted_edges():
            for i in physical.nodes():
                mapping_ILP += pulp.lpSum(
                    ...
                    physical.neighbors(i) for device in physical.nw_interfaces(i, j)) == \
        ...
        for (i, j, device) in physical.edges(keys=True):
            mapping_ILP += pulp.lpSum(virtual.req_rate(u, v) * (
                    link_mapping[(u, v, i, j, device)] + link_mapping[(u, v, j, i, device)])
                                      for (u, v) in virtual.sorted_edges()) <= physical.rate(i, j, device), \
        ...
        res_node_mapping, res_link_mapping = self._build_ILP_solution(virtual, physical, node_mapping, link_mapping)
        # if interfaces have been grouped, map to solution to the original network
        solution = Solution.build_solution(virtual, physical, res_node_mapping, res_link_mapping)
...
if __name__ == "__main__":
    ...
    time_solution, solution = embed_ilp(virtual_topo, physical_topo)
